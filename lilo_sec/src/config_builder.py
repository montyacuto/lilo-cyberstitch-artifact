...
LIBRARY_NAMER_GPT_PARAM_KEYS = {
    "body_task_selection",
    "best_of",
    "n_samples_per_abstraction",
    "top_p",
    "max_tokens",
    "n_usage_examples",
    "resume_strategy",
...
class ExperimentType(str, Enum):
    ...
    LILO_ORIGINAL_COMPLETION = "lilo_original_completion"
    LLM_SOLVER_ORIGINAL_COMPLETION = "llm_solver_original_completion"
    RANDOM = "random"
...
def build_config(
    ...
    max_mem_per_enumeration_thread: int = None,
    encoder: str = None,
...
def build_config_body(
    ...
    for block in config["experiment_iterator"]["loop_blocks"]:
        if (
            ...
            if max_mem_per_enumeration_thread is not None:
                block["params"].update(
                    {
                        "max_mem_per_enumeration_thread": max_mem_per_enumeration_thread,
                    }
        ...
        if block.get("model_type") == LIBRARY_NAMER:
            ...
            _gpt_params.update(
                {
                    k: v
                    for k, v in gpt_params.items()
                    if k in LIBRARY_NAMER_GPT_PARAM_KEYS
                }
            )
