...
class GPTBase(object):
    ...
    def __init__(self, experiment_state=None, engine=None):
        super().__init__()
        configure_api_key(os.getenv("OPENAI_API_KEY"))

        self.ENGINE = engine or self.ENGINE_DEFAULT
        self.ENGINE_MAX_TOKENS = self.MAX_TOKENS_PER_ENGINE.get(
            self.ENGINE,
            int(os.getenv("LILO_ENGINE_MAX_TOKENS", self.ENGINE_MAX_TOKENS_FALLBACK)),
        )
    ...
    def is_chat_format(self):
        engine = resolve_model_alias(self.ENGINE).split("/", 1)[-1]
        if engine.endswith("-instruct"):
            return False
        return engine in self.CHAT_ENGINES or engine.startswith("gpt-")
